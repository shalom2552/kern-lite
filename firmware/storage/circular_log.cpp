...
// A slot past EOF is unwritten: return a zeroed record (CRC fails) rather
// than expanding the file. False return means I/O error.
...
StorageStatus CircularLog::mount()
{
    ...
    if (readMeta() == StorageStatus::Ok) {
        // Catch up on records written since the last meta flush.
...
uint32_t CircularLog::totalRecords() const
{
    return m_meta.total_records;
}

uint32_t CircularLog::wrapCount() const
{
    return m_meta.wrap_count;
}

uint8_t CircularLog::currentFile() const
{
    return m_meta.current_file;
}

uint16_t CircularLog::writeIndex() const
{
    return m_meta.write_index;
}

bool CircularLog::isMounted() const
{
    return m_mounted;
}
...
StorageStatus CircularLog::recoverPosition()
{
    ...
    // Head is the slot after the newest record. Full ring means it has wrapped.
